<header class="bg-primary-800 border-b-2 border-accent-orange sticky top-0 z-40">
    <div class="max-w-6xl mx-auto px-4 py-4">
        <div class="flex items-center justify-between">
            <div class="flex items-center">
                <a href="{{ url('/') }}" class="flex items-center gap-2 group">
                    <svg class="w-7 h-7 fill-accent-orange group-hover:fill-accent-lightOrange transition-colors duration-200" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <span class="text-2xl font-bold text-accent-orange group-hover:text-accent-lightOrange transition-colors duration-200">
                        Critical Dice
                    </span>
                </a>
            </div>
            ...
            <nav class="hidden md:flex items-center gap-8">
                <a href="{{ url('/') }}" class="text-gray-300 hover:text-accent-orange transition font-semibold">
                    Inicio
                </a>

                <div class="relative group">
                    <button class="text-gray-300 hover:text-accent-orange transition font-semibold flex items-center gap-2">
                        Personajes
                    ...
                    <div class="absolute left-0 mt-0 w-48 bg-primary-700 border-2 border-accent-orange rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
                        <a href="{{ url('/character') }}" class="block px-4 py-3 text-gray-300 hover:text-white hover:bg-primary-600 font-semibold border-b border-primary-600 first:rounded-t-lg">
                            Crear Personaje Nuevo
                        </a>
                        <a href="{{ url('/characters') }}" class="block px-4 py-3 text-gray-300 hover:text-white hover:bg-primary-600 font-semibold last:rounded-b-lg">
                            Mis Personajes
                        </a>
                    </div>
                </div>
            ...
            <div class="flex items-center gap-4">
                ...
                <div class="flex items-center gap-4">
                    <a href="{{ route('dashboard') }}" class="text-gray-300 hover:text-accent-orange transition font-semibold">
                        Dashboard
                    </a>
                    <span class="text-gray-400">Hola, <span class="text-accent-orange font-semibold">{{ Auth::user()->name }}</span></span>
                    <form method="POST" action="{{ route('logout') }}" class="inline">
                        @csrf
                        <button type="submit" class="px-4 py-2 bg-red-600 hover:bg-red-700 text-white font-bold rounded-lg transition">
                            Cerrar sesión
                        </button>
                    </form>
                </div>
                @else
                {{-- Usuario NO autenticado: mostrar registro/login --}}
                <a href="{{ route('register') }}" class="px-4 py-2 bg-accent-orange hover:bg-accent-darkOrange text-white font-bold rounded-lg transition">
                    Registrarse
                </a>
                <a href="{{ route('login') }}" class="px-4 py-2 bg-primary-700 hover:bg-primary-600 text-gray-300 font-bold rounded-lg transition border border-gray-600">
                    Iniciar sesión
                </a>
                @endauth

                <button class="md:hidden text-accent-orange hover:text-white transition">
